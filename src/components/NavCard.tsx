...
const NavCard : React.FC<NavCardProps> = ({ href, title, subtitle, description, icon, delay }) => {
    return (
        <Link
            href={href}
            className={`group relative flex flex-col p-8 h-80 rounded-3xl bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 shadow-sm hover:shadow-xl hover:border-blue-500/30 transition-all duration-500 animate-in fade-in slide-in-from-bottom-10 fill-mode-backwards ${delay}`}
        >
